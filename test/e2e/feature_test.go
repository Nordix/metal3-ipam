...
var _ = Describe("Metal3 IPAM advanced features", Label("ipam", "features"), func() {
	...
	Context("Preallocation", func() {
		It("Should allocate preallocated IPs to multiple matching claims", func() {
			...
			claimName1 := fmt.Sprintf("test-prealloc-multi-1-%d", GinkgoParallelProcess())
			claimName2 := fmt.Sprintf("test-prealloc-multi-2-%d", GinkgoParallelProcess())
			...
			ipPool := createIPPoolWithPreAllocations(ctx, cl, "test-ippool-prealloc-multi", map[string]ipamv1.IPAddressStr{
				claimName1: prealloc1,
				claimName2: prealloc2,
			})

			By("Creating both IPClaims")
			claim1 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, claimName1)
			claim2 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, claimName2)
			...
			Expect(cl.Delete(ctx, claim2)).To(Succeed())
			Eventually(func() bool {
				return apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(claim1), &ipamv1.IPClaim{})) &&
					apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(claim2), &ipamv1.IPClaim{}))
			}, e2eConfig.GetIntervals(specName, "wait-ippool")...).Should(BeTrue())
	...
	Context("Annotation-based IP request", func() {
		It("Should allocate the specific IP requested via Metal3 IPClaim annotation", func() {
			...
			ipPool := createIPPool(ctx, bootstrapClusterProxy, "test-ippool-annot-m3")

			By("Creating an IPClaim with ipAddress annotation")
			requestedIP := "192.168.1.77"
			claimName := fmt.Sprintf("test-annot-claim-%d", GinkgoParallelProcess())
			...
			Eventually(func() bool {
				return apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(ipClaim), &ipamv1.IPClaim{}))
			}, e2eConfig.GetIntervals(specName, "wait-ippool")...).Should(BeTrue())
			Expect(cl.Delete(ctx, ipPool)).To(Succeed())
		})
		...
		It("Should allocate the specific IP requested via CAPI IPAddressClaim annotation", func() {
			...
			By("Creating an IPPool")
			ipPool := createIPPool(ctx, bootstrapClusterProxy, "test-ippool-annot-capi")

			By("Creating a CAPI IPAddressClaim with ipAddress annotation")
			requestedIP := "192.168.1.88"
			claimName := fmt.Sprintf("test-capi-annot-claim-%d", GinkgoParallelProcess())
			...
			Eventually(func() bool {
				return apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(claim), &capipamv1.IPAddressClaim{}))
			}, e2eConfig.GetIntervals(specName, "wait-ippool")...).Should(BeTrue())
			Expect(cl.Delete(ctx, ipPool)).To(Succeed())
		})
	})
	...
	Context("Pool exhaustion and recovery", func() {
		It("Should handle pool exhaustion and recover when IPs are freed", func() {
			...
			claim1Name := fmt.Sprintf("test-exhaust-claim-1-%d", GinkgoParallelProcess())
			claim2Name := fmt.Sprintf("test-exhaust-claim-2-%d", GinkgoParallelProcess())
			claim1 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, claim1Name)
			claim2 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, claim2Name)
			...
			claim3Name := fmt.Sprintf("test-exhaust-claim-3-%d", GinkgoParallelProcess())
			claim3 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, claim3Name)
			...
			claim4Name := fmt.Sprintf("test-exhaust-claim-4-%d", GinkgoParallelProcess())
			claim4 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, claim4Name)
			...
			By("Cleaning up")
			Expect(cl.Delete(ctx, claim2)).To(Succeed())
			Expect(cl.Delete(ctx, claim4)).To(Succeed())
			Eventually(func() bool {
				return apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(claim2), &ipamv1.IPClaim{})) &&
					apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(claim4), &ipamv1.IPClaim{}))
			}, e2eConfig.GetIntervals(specName, "wait-ippool")...).Should(BeTrue())
			Expect(cl.Delete(ctx, ipPool)).To(Succeed())
		})
	})
	...
	Context("Multiple pools in one IPPool", func() {
		It("Should allocate from second pool when first is exhausted", func() {
			...
			claim1 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-multi-1-%d", GinkgoParallelProcess()))
			Eventually(func(g Gomega) {
				retrieved := &ipamv1.IPClaim{}
				g.Expect(cl.Get(ctx, client.ObjectKeyFromObject(claim1), retrieved)).To(Succeed())
				g.Expect(retrieved.Status.Address).ToNot(BeNil())
			...
			claim2 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-multi-2-%d", GinkgoParallelProcess()))
			Eventually(func(g Gomega) {
				retrieved := &ipamv1.IPClaim{}
				g.Expect(cl.Get(ctx, client.ObjectKeyFromObject(claim2), retrieved)).To(Succeed())
				g.Expect(retrieved.Status.Address).ToNot(BeNil())
	...
	Context("Pool-level gateway and DNS override", func() {
		It("Should use pool-level gateway and DNS when set, falling back to spec-level", func() {
			...
			claim1 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-override-1-%d", GinkgoParallelProcess()))
			Eventually(func(g Gomega) {
				retrieved := &ipamv1.IPClaim{}
				g.Expect(cl.Get(ctx, client.ObjectKeyFromObject(claim1), retrieved)).To(Succeed())
				g.Expect(retrieved.Status.Address).ToNot(BeNil())
			...
			claim2 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-override-2-%d", GinkgoParallelProcess()))
			Eventually(func(g Gomega) {
				retrieved := &ipamv1.IPClaim{}
				g.Expect(cl.Get(ctx, client.ObjectKeyFromObject(claim2), retrieved)).To(Succeed())
				g.Expect(retrieved.Status.Address).ToNot(BeNil())
	...
	Context("IPPool status tracking", func() {
		It("Should track allocations in IPPool status", func() {
			...
			By("Creating an IPPool")
			ipPool := createIPPool(ctx, bootstrapClusterProxy, "test-ippool-status")

			By("Creating two IPClaims")
			claim1 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-status-1-%d", GinkgoParallelProcess()))
			claim2 := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-status-2-%d", GinkgoParallelProcess()))
	...
	Context("Mixed Metal3 and CAPI claims on same pool", func() {
		It("Should allocate unique IPs for both Metal3 and CAPI claims from the same pool", func() {
			...
			By("Creating an IPPool")
			ipPool := createIPPool(ctx, bootstrapClusterProxy, "test-ippool-mixed")

			By("Creating a Metal3 IPClaim and a CAPI IPAddressClaim on the same pool")
			m3ClaimName := fmt.Sprintf("test-mixed-m3-%d", GinkgoParallelProcess())
			capiClaimName := fmt.Sprintf("test-mixed-capi-%d", GinkgoParallelProcess())
			m3Claim := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, m3ClaimName)
			capiClaim := createCAPIIPAddressClaim(ctx, bootstrapClusterProxy, ipPool.Name, capiClaimName)
			...
			By("Cleaning up")
			Expect(cl.Delete(ctx, m3Claim)).To(Succeed())
			Expect(cl.Delete(ctx, capiClaim)).To(Succeed())
			Eventually(func() bool {
				return apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(m3Claim), &ipamv1.IPClaim{})) &&
					apierrors.IsNotFound(cl.Get(ctx, client.ObjectKeyFromObject(capiClaim), &capipamv1.IPAddressClaim{}))
			}, e2eConfig.GetIntervals(specName, "wait-ippool")...).Should(BeTrue())
			Expect(cl.Delete(ctx, ipPool)).To(Succeed())
		})
	})
	...
	Context("IPPool deletion with active allocations", func() {
		It("Should block IPPool deletion via finalizer while claims exist", func() {
			...
			ipPool := createIPPool(ctx, bootstrapClusterProxy, "test-ippool-finalizer")
			claim := createIPClaim(ctx, bootstrapClusterProxy, ipPool.Name, fmt.Sprintf("test-finalizer-%d", GinkgoParallelProcess()))
			...
			}, e2eConfig.GetIntervals(specName, "wait-ippool")...).Should(BeTrue(), "IPPool should be deleted after claims are removed")
		})
	})
})

// createIPPoolWithPreAllocations creates an IPPool with preAllocations set.
func createIPPoolWithPreAllocations(ctx context.Context, cl client.Client, name string, preAllocations map[string]ipamv1.IPAddressStr) *ipamv1.IPPool {
	Logf("Creating IPPool %s with %d preallocations", name, len(preAllocations))
