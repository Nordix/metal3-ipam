...
| Command | Purpose |
|---------|---------|
| `make test-e2e` | Run all E2E tests (bootstraps Kind cluster) |
| `make test-e2e GINKGO_FOCUS_LABELS=basic` | Run basic scenario only |
| `make test-e2e GINKGO_FOCUS_LABELS=features` | Run feature scenarios only |
| `make test-e2e USE_EXISTING_CLUSTER=true` | Run against an existing cluster |

Key files:

- `test/e2e/config/e2e_conf.yaml` - Provider versions and test config
- `test/e2e/data/shared/` - Provider metadata per version
